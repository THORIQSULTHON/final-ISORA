...
?>
	<div class="container">
		<div class="content">
			<h2>Data Guru &raquo; Ganti Password</h2>
			<hr />
			
			<p>Ganti password Guru dengan NIP <?php echo '<b>'.$_GET['nip'].'</b>'; // mengambil nilai nip dari data yang dipilih ?></p> 
			...
			if(isset($_POST['ganti'])){ // jika tombol 'Simpan' dengan properti name="ganti" pada baris 64 ditekan
				...
				$cek = mysqli_query($koneksi, "SELECT * FROM tb_guru WHERE nip='$nip'"); // query memilih nis dan password
				if(mysqli_num_rows($cek) == 0){ // mengecek query $cek jika password yang dimasukkan tidak sesuai dengan nip
					...
					if($password1 == $password2){ // jika nilai password1 dan password2 bernilai sama
						if(strlen($password1) >= 6){ // mengecek panjang password minimal 6 karakter
							...
							$update = mysqli_query($koneksi, "UPDATE tb_guru SET password='$pass' WHERE nip='$nip'"); // query update password dari nip yang dipilih
